...
class LeaseStore:
    ...
    # ---- load ----
    def load(self, grace: bool = True) -> Dict[str, Lease]:
        """Populate the in-memory cache from the DB.

        Each loaded lease normally gets a *grace*: ``last_heartbeat`` is reset
        to now, so a restart doesn't immediately reap still-valid leases
        (agents get time to re-send heartbeats). Pass ``grace=False`` to keep
        the stored ``last_heartbeat`` verbatim — required by the reaper
        (``reap_expired_leases`` / ``bin/reap-leases.py``), which must judge
        expiry against the real last-seen heartbeat, not a fresh restart
        grace. If the DB is empty but a legacy ``leases.json`` exists beside
        it, perform a one-time migration that preserves existing leases (e.g.
        ``sh`` -> 8100).
        """
        self._cache = {}
    ...
    # ---- reaper: release orphaned / dead-agent leases ----
    def reap_expired_leases(self, now: Optional[float] = None) -> int:
        """Release leases whose heartbeat deadline has passed.

        A lease is expired when ``last_heartbeat + lease_timeout < now``. This
        recovers capacity from agents that died without calling ``release``
        (e.g. crash/OOM): without a reaper such leases accumulate forever in
        the durable store.

        The store never schedules anything itself — call this on demand (see
        ``bin/reap-leases.py``) or register a periodic call via gatekeeper's
        own ``register_timer`` tool if you want it automated.

        Args:
            now: epoch seconds (``time.time()``); injected for testing.
        ...
        expired = [
            ...
            if (l.last_heartbeat + l.lease_timeout) < now
        ]
        for rid in expired:
            # delete() removes from cache + DB; snapshot once at the end.
            self.delete(rid, _snapshot=False)
        if expired:
            self.snapshot()
        return len(expired)
